"""
HTML → Markdown 提取 v6（真正的最终版）
关键修复:
- 从 NOISE_TAGS 里移除 <header> 和 <aside>
  原因: 这两个标签含义太歧义，TikTok 用 <header> 作为 section 标题容器
  噪音 class 已经能精确处理真正的导航/侧边栏
- 保留 <nav> 删除（HTML5 里 <nav> 含义无歧义）
"""
...
REPORT_PATH = Path("data/processed/11_extraction_report_v6.csv")
ERROR_LOG_PATH = Path("data/processed/11_extraction_errors_v6.txt")

# 噪音标签：只删除含义无歧义的
# 移除了 <header>, <aside>, <footer>（footer 也可能是 article footer 含 last-updated 之类）
NOISE_TAGS = ["script", "style", "nav", "noscript", "iframe", "svg", "img"]

# 噪音 class：精确匹配，已经覆盖了真正的导航/侧边栏/页脚
NOISE_CLASS_EXACT = {
...
def main():
    print("第三步：HTML → Markdown 提取 v6（最终版）")
    ...
    if len(success) > 0:
        ...
        for cat, group in df.groupby("category"):
            ok = (group["extracted_chars"] > 0).sum()
            print(f"  {cat}: {ok}/{len(group)} ({ok/len(group)*100:.1f}%)")
