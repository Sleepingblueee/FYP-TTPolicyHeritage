...
HTML → Markdown 提取 v4
关键修复:
1. 噪音 class 过滤改用词边界匹配，避免误伤 page-with-header 这种 class
2. 标记 CSR-only 文件 (body 文本 < 6000 字符且主要是导航词)
3. 增加质量审计标记
"""
...
REPORT_PATH = Path("data/processed/11_extraction_report_v4.csv")
ERROR_LOG_PATH = Path("data/processed/11_extraction_errors_v4.txt")

# 噪音标签：直接删
NOISE_TAGS = ["script", "style", "nav", "footer", "header",
              "noscript", "iframe", "svg", "img", "aside"]

# 噪音 class 模式：使用词边界匹配（不会再误伤 page-with-header）
NOISE_CLASS_PATTERNS = [
    r"^nav$", r"-nav$", r"^nav-", r"_nav$", r"^nav_",
    r"^header$", r"-header$", r"^header-", r"header_",
    r"^footer$", r"-footer$", r"^footer-", r"footer_",
    r"^sidebar$", r"-sidebar$", r"^sidebar-",
    r"side-list",
    r"cookie-banner", r"cookie-consent",
    r"skip-link", r"skiplink",
    r"language-selector", r"region-selector",
    r"wb-autocheck", r"wm-",
    r"breadcrumb",
]
NOISE_CLASS_REGEX = re.compile("|".join(NOISE_CLASS_PATTERNS), re.IGNORECASE)
...
def is_csr_only(html, soup):
    """判断这个文件是不是 CSR 壳（body 短 + 主要是导航 + 含大量 JS）"""
    body = soup.find("body")
    if not body:
        return False
    body_text = body.get_text(strip=True)
    
    # 标志 1: body 总文本 < 6000 字符（典型 CSR 壳是 5000-5500）
...
def remove_noise(soup):
    ...
    for elem in list(soup.find_all(attrs={"class": True})):
        ...
        # 对每个 class 名单独检查（不是整段拼起来后子串匹配）
        if any(NOISE_CLASS_REGEX.search(c) for c in cls_list):
            elem.decompose()
...
def try_method_main_body(soup):
    """新方法: 找 class 含 'main-body' 或 'tiktok-web-article' 的 div (TRANS 等用)"""
    candidates = soup.find_all("div", class_=re.compile(r"(main-body|tiktok-web-article|main-content)"))
    for c in candidates:
...
def extract_one(filepath, is_news=False):
    try:
        ...
        # 顺序: A → article-wrapper → main-body → B → C
